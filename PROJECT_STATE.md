...
Date: 2026-06-17 (original review) · **Last refreshed: 2026-08-08**
...
> - 🔧 **Open ops items:** **DMI-45** (hashrate shortfall — diagnosed, now a hardware-repair list) · **DMI-46** (2026-08-07 outage — largely answered: the 4G uplink is measurably unstable) · **DMI-47..51** (site fault-tolerance; **DMI-49's watchdog written and pushed**, unmerged) · **DMI-52** (miner error codes / ambient temp / fan RPM as metrics) · **DMI-53** (router log flooded by a broken AdGuard hook).
> - 🚨 **Site unreachable since 2026-08-07 23:30** — dead 4G uplink, needs a physical power-cycle. Miners keep hashing; monitoring and remote access are what's lost. See DMI-46 below.
...
- **DMI-46** — post-mortem of the 2026-08-07 outage. Originally undetermined, because the pre-reboot router log was lost. **Largely answered the same evening by DMI-44's first real harvest**, which showed the uplink failing repeatedly and unattended:
  - 17:59→18:03, 20:26→20:47, 21:51→22:13, 22:39→~23:02 — four outages in five hours, ~70 min down, **~77% availability**, each recovering by itself in a strikingly consistent 21–23 min.
  - Then a fifth, starting 23:30, which **did not recover at all** — still down 11+ hours later, and matching the daytime failure that needed a physical power-cycle on site.
  - Throughout, **neither the router nor the Pi rebooted**: the router kept ACKing LAN DHCP mid-outage, the Pi's `uptime` spanned the whole period, and containers show months of uptime. The failure is isolated to the 4G uplink.
  - Conclusion: the `ip ssh security-level` hypothesis is no longer *needed* (not disproven — just no longer exotic). This link demonstrably drops on its own.
  - Two clean regimes, nothing in between: recover in ~20 min, or not at all. That gap is what the DMI-49 threshold targets.
  - Caveat found: `Network::InternetChecker` missed one recovery entirely, so it cannot be a watchdog's only signal.
...
**⚠️ The site has been unreachable since 2026-08-07 23:30** (router, Pi and monitoring all behind the dead uplink; the miners themselves keep hashing and the owners' pool-side stats are unaffected). Nothing will restore it automatically — `ping-check` auto modem-restart is disabled by design here — so it needs a physical power-cycle of the router and modem. Blocked on that: the router Entware inspection + DMI-53 removal, the DMI-45 inventory insert, and any deploy.
