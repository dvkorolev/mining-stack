...
Date: 2026-06-17 (original review) · **Last refreshed: 2026-08-13**
Reviewer: Claude Code
Scope: full repository read (`backend/`, `frontend/`, `python-scheduler/`, Docker/monitoring, deploy scripts).

> **Status at a glance (main `2d6cf5e`, deployed and verified on the Pi)**
...
> - 🔧 **Open ops items:** **DMI-62** (per-board metrics fabricated as zeros on 18 of 19 miners; surfaces as 21 false `MinerMissingChips`) · **DMI-55** (the stale-series cull removes the wrong metrics) — both investigated to root cause 2026-08-13 and **deferred by the owner**, findings recorded on the tickets · **DMI-47** (uplink observability: exporter + alert shipped, SIM/session state still unmeasured) · **DMI-45** (hashrate shortfall — a hardware-repair list) · **DMI-52** (error codes / ambient temp / fan RPM) · **DMI-57** (retest the Fibocom + external antenna) · DMI-50/51 (remaining fault-tolerance layers).
...
- **DMI-55** — dead miners' gauges live in the scheduler's process memory and are never cleared, so a machine that stopped responding weeks ago still contributes its last hashrate, power and temperature to every fleet aggregate. Deliberately split from DMI-54 to keep that a pure crash fix: this one changes dashboard numbers and alert inputs, and needs a decision on semantics rather than a mechanical patch. **Investigated 2026-08-13, then deferred by the owner; answers recorded on the ticket.**
  - 🚨 **The existing cull is inverted.** `main.py:562` clears `[miner_scrape_status, miner_state]` after `FAILURE_THRESHOLD = 5` failures (10 min at `COLLECTION_INTERVAL = 2`) — exactly the two series that say *"this machine is known and is not answering"* — and leaves the eleven value-carrying ones publishing their last good readings forever.
  - **This is also the cause of the 20↔25 series oscillation** that the DMI-58 note warns about as a separate annoyance. Keeping `miner_scrape_status = -2` and dropping the rest would stabilise the count at 25 *and* correct the aggregates, in one change.
  - *Answers to the ticket's three open questions:* keep `miner_scrape_status`, remove everything else including `miner_state` (an unknown state must be absent, not zero); reuse the existing threshold, since removing a series does not lose history — the TSDB keeps it; and the two consumers found both improve — `sum(miner_hashrate_ths{algorithm="sha256"}) < 1500` starts summing reality, and the backend's `getMinerStatus()` (`prometheus.service.ts:181`) correctly drops a dead miner. No rule uses `absent()`.
  - ⚠️ `get_all_miner_metrics()` (`metrics.py:160-177`) lists 14 gauges and **omits the per-board and per-fan ones**, which carry a different label set (`slot`, `fan_id`) — the same 162 fabricated series as DMI-62. **The two tickets meet here and the removal mechanism should be built once for both.** The helper already does the hard part: `remove_miner_series(ip, metrics=None)` defaults to every miner gauge and recovers the full label set from the cache.
  - *Live state when checked:* no inflation present — all 20 polled miners answering, zero orphaned series, `sum(miner_hashrate_ths) = 2006.1` genuine. The bug is latent and fires when a previously healthy miner dies, which makes now a convenient time to change it.
...
- **DMI-62** — 🔧 **OPEN. Investigated to root cause 2026-08-13, then deferred by the owner; findings are on the ticket.** The visible symptom is `MinerMissingChips` producing **21 permanently-firing false positives**, roughly half the alert list. The actual scope is wider, and the ticket's own suggested fix does not work.
  - **The whole per-board layer is fabricated zeros on 18 of 19 polled miners.** Not just chips: `miner_board_hashrate_ths` and `miner_board_temp_c` are zero too, all three slots each, while 16 of those machines are in state `mining` and the fleet does ~2000 TH/s. A board reporting 0 °C on a hashing miner is not a measurement. 162 fake series; exactly one machine (`rebuildm30`) reports anything real — 35 TH/s, 87 °C, 78/78 chips.
  - **Root cause is one idiom:** `pyasic_collector.py:350-356` publishes `board.hashrate or 0`, `board.chips or 0`, `board.expected_chips or 0`. `or 0` turns "not reported" into "zero". The `hasattr` guards beside them are decorative — the attribute always exists on a pyasic `HashBoard`; only its value is `None`. This is rule 2 above, violated in the collector.
  - ⚠️ **The ticket's suggested approach #2 cannot work as written.** It proposes gating the rule on a healthy per-board hashrate to distinguish "not reported" from "no chips" — but per-board hashrate is zero on all 18, so the proposed discriminator is broken by the same bug. Checked before implementing.
  - The 11 miners reading `0/0` are not healthier than the 7 that alert; they have the same missing data and are silent only because `chips_expected` is unpopulated too. The quiet half is the larger one.
  - *Proposed fix (not implemented):* publish a board gauge only when the value is not `None`, otherwise remove the child series — the 18 lose their series, `rebuildm30` keeps real ones, and a genuine zero on a dead board still alerts. Plus `miner_board_chips_expected > 0` on the rule as belt-and-braces. Blast radius is small: **nothing consumes these four metrics except this one alert** — no dashboard panel references `miner_board_*`.
  - *Separate question:* why pyasic returns `HashBoard` objects with slots but no contents on 18 of 19 miners. Needs a live probe of one miner's raw API response.
  - Why it still matters: a list that is half noise trains everyone to stop reading it, and the list is the only thing standing between a real fault and a silent one. Same class as the `BackupPoolDead` severity decision above, and worth fixing before any new alert is added.
...
2. **DMI-62 + DMI-55 together — one removal mechanism, two tickets.** Both are the same defect
   (fabricated zeros / the wrong series culled) and both are already investigated to root cause with
   the design decisions settled on the tickets. Doing them separately means writing series removal
   twice. Purely repo work; deferred by the owner on 2026-08-13, so pick up when convenient.
