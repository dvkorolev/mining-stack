...
> **Status at a glance (main `2a4bff3`, deployed and verified on the Pi)**
...
3. **DMI-45 — the inventory half is done; what is left needs hands, not SQL.** The identity drift is
   fixed and verified (see below), so the remaining items are the physical ones — `.117`'s dead fans,
   `.58`'s hashboards — plus three decisions that are the owner's: the three phantom `192.168.1.x`
   MACs, whose only evidence is the last octet of a name; the `.121`/`.122` name collision, where the
   database name follows the slot and the pool follows the machine; and what to do with the five
   machines that are physically absent and correctly firing `MinerOffline`.
